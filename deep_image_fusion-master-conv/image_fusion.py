...
# Set the image number to 21
id_x = 12

#Read the visible and infrared images
gray_img = imread(f'IV_images/VIS{id_x}.png')
ir_img = imread(f'IV_images/IR{id_x}.png')

pic = fuse(gray_img, ir_img)

# Save the fused image
pict = pic * 255  # Scale the image for saving
pict = pict.astype(np.uint8)
imsave(f"fused_image/fused{id_x}.png", pict)
...
plt.imshow(pic, 'gray')
plt.axis('off')
plt.title(f'Fused Image')  # Displaying fused image title with id_x
